...
// Regenerate all Dropbox URLs for images with empty URLs
app.post('/api/admin/regenerate-urls', async (req, res) => {
  try {
    console.log('🔗 Starting URL regeneration for images...');
    
    // Get images with empty or null URLs
    const result = await databaseService.query(`
      SELECT id, filename, dropbox_path, dropbox_id 
      FROM images 
    ...
    console.log(`📊 Found ${images.length} images without URLs`);
    ...
    for (const image of images) {
      try {
        console.log(`🔗 Generating URL for: ${image.filename}`);
        
        // Generate temporary link using Dropbox service
        const url = await dropboxService.getTemporaryLink(image.dropbox_path);
        ...
        errorCount++;
        const errorMsg = `${image.filename}: ${error.message}`;
        errors.push(errorMsg);
        console.error(`❌ Failed to generate URL for ${image.filename}:`, error.message);
      }
    }
    
    console.log(`🎉 URL regeneration complete: ${successCount} success, ${errorCount} errors`);
    ...
    console.error('❌ URL regeneration error:', error);
    res.status(500).json({ error: 'URL regeneration failed: ' + error.message });
  }
});
